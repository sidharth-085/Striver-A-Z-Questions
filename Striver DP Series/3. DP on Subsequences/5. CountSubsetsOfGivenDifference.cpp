...
// In this question, we have to find the number of subset pairs having difference
// of their sums equal to given diff. Here we have constraints starting from 0. 
// So we have to change the count subsets function little bit as discussed in 
// previous question. 
...
int countSubsets(vector<int> &arr, int n, int k) {
    ...
    for (int index = 1; index < n; index++) {
        ...
        for (int sum = 0; sum <= k; sum++) {
            ...
            currRow[sum] = (include + exclude) % mod;
        }
