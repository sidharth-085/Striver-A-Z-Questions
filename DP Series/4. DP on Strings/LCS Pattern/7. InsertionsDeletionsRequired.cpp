...
// This problem is same as Edit Distance Problem, just do Insertion and Deletion
// operations on the string text1 to make text2
...
int helper(int n, int m, string &text1, string &text2) {
    ...
    return min(deleteChar, insertChar);
}
...
int helper(int n, int m, string &text1, string &text2, vector<vector<int>> &dp) {
    ...
    return dp[n][m] = min(deleteChar, insertChar);
}
    ...
    for (int n = 1; n <= size1; n++) {
        for (int m = 1; m <= size2; m++) {
            ...
            if (text1[n - 1] == text2[m - 1]) {
                ...
                dp[n][m] = min(deleteChar, insertChar);
            }
        }
    }
    ...
    for (int n = 1; n <= size1; n++) {
        ...
        for (int m = 1; m <= size2; m++) {
            ...
            if (text1[n - 1] == text2[m - 1]) {
                ...
                curr[m] = min(deleteChar, insertChar);
            }
        }
